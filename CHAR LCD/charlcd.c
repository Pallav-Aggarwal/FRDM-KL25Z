...
void send_lcd(unsigned char data_byte, unsigned char type)
{
    ...
    if(type == COMMAND)
    {
        CLRBIT(RS_PORT_CLR, RS_PIN);
    }
    else if(type == DATA)
    {
        SETBIT(RS_PORT_SET, RS_PIN);
    }

#if LCD_INTERFACE == EIGHT_BIT
    LCD_PORT = data_byte;
    SETBIT(EN_PORT_SET,EN_PIN);delay(LCD_EN_DELAY);
    CLRBIT(EN_PORT_CLR,EN_PIN);delay(LCD_EN_DELAY);
    ...
    //Higher Nibble
    if(data_byte & 0x10) SETBIT(D4_PORT_SET,D4_PIN);
    else CLRBIT(D4_PORT_CLR,D4_PIN);
    if(data_byte & 0x20) SETBIT(D5_PORT_SET,D5_PIN);
    else CLRBIT(D5_PORT_CLR,D5_PIN);
    if(data_byte & 0x40) SETBIT(D6_PORT_SET,D6_PIN);
    else CLRBIT(D6_PORT_CLR,D6_PIN);
    if(data_byte & 0x80) SETBIT(D7_PORT_SET,D7_PIN);
    else CLRBIT(D7_PORT_CLR,D7_PIN);

    SETBIT(EN_PORT_SET,EN_PIN);delay(LCD_EN_DELAY);
    CLRBIT(EN_PORT_CLR,EN_PIN);delay(LCD_EN_DELAY);

    //Lower Nibble
    if(data_byte & 0x01) SETBIT(D4_PORT_SET,D4_PIN);
    else CLRBIT(D4_PORT_CLR,D4_PIN);
    if(data_byte & 0x02) SETBIT(D5_PORT_SET,D5_PIN);
    else CLRBIT(D5_PORT_CLR,D5_PIN);
    if(data_byte & 0x04) SETBIT(D6_PORT_SET,D6_PIN);
    else CLRBIT(D6_PORT_CLR,D6_PIN);
    if(data_byte & 0x08) SETBIT(D7_PORT_SET,D7_PIN);
    else CLRBIT(D7_PORT_CLR,D7_PIN);
